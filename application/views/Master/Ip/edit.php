							...
							<div class="col-sm-12">
								<input type="hidden" name="<?php echo $this->security->get_csrf_token_name(); ?>" value="<?php echo $this->security->get_csrf_hash(); ?>">
		  						<div class="form-group d-none">
			  						<label>id</label>
			  						<input required readonly type="text" name="id" class="hide form-control" title="Harus di isi" value="<?=$data->ip_id?>">
			  					</div>
								<div class="form-group">
									<label>Ip Address</label>
									<input required name="ip_address" class="form-control" value="<?=$data->ip_address?>" />
								</div>
                                <div class="form-group">
                                    <label>Kategori</label>
                                    <textarea class="form-control" name="ip_catatan" rows="8"><?=$data->ip_catatan?></textarea>
                                </div>
							</div>
						</div>
			  			<div class="row">
