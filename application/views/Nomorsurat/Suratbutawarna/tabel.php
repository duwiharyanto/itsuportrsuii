<div class="row">
    <div class="col-lg-12">
      <div class="alert alert-danger alert-dismissable">
        ...
        Format Nomor : (NO.SURAT / B.1 / RSUII / BULAN ROMAWI/ TAHUN)
      </div>        
            ...
            <div class="table-responsive">
                <table id="example23" class="display nowrap table table-striped" cellspacing="0" width="100%">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td><?=$i?></td>
                      <td><?=$row->suratbutawarna_nomor?><br><small class="text-danger">Disimpan oleh :<?=ucwords($row->user_nama)?><br>Disimpan : <?=date('d-m-Y',strtotime($row->created_at))?></small></td>
                      <td><?=ucwords($row->suratbutawarna_bulan)?></td>
                      <td><?=ucwords($row->suratbutawarna_norm)?></td>
                      <td><?=ucwords($row->suratbutawarna_nama)?></td>
                      <td class="text-center">
                        <?php tombolaksi($global,$row->suratbutawarna_id,$this->uri->segment(3))?>
                      </td>
                    </tr>
                  <?php $i++;?>  
                  <?php endforeach;?>
