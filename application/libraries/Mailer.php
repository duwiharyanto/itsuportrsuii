...
class Mailer {
    protected $_ci;
    protected $email_pengirim = 'itsuportrsuii@gmail.com'; // Isikan dengan email pengirim
    protected $nama_pengirim = 'IT Support RS UII'; // Isikan dengan nama pengirim
    protected $password = 'r5u11bantul'; // Isikan dengan password email pengirim
    ...
    public function send_with_attachment($data){
        ...
        // $mail->SMTPDebug = 2; // Aktifkan untuk melakukan debugging
        $mail->setFrom($this->email_pengirim, $this->nama_pengirim);
        foreach ($data['email_penerima'] as $val) {
           $mail->addAddress($val);
        }
        ...
        if(filesize($data['attachment']) <= 25000000){ // Jika ukuran file <= 25 MB (25.000.000 bytes)
