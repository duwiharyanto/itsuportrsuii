...
class Keterangandiagnosa extends CI_Controller {
// class Registrasi extends Core {
	...
	//VARIABEL
	private $master_tabel="keterangandiagnosa"; //Mendefinisikan Nama Tabel
	private $id="keterangandiagnosa_id";	//Menedefinisaikan Nama Id Tabel
	private $kodeberkas="B.18";
	private $filename='Surat Keterangan Diagnosa';
	private $namainstansi='RSUII';
	private $default_url="Nomorsurat/Keterangandiagnosa/"; //Mendefinisikan url controller
	private $default_view="Nomorsurat/Keterangandiagnosa/"; //Mendefinisiakn defaul view
	...
	private function global_set($data){
		...
		if(isset($data['overwriteview'])){
			...
			$overwriteview="views/Nomorsurat/Keterangandiagnosa/index.php";
			$menu_submenu="keterangan_diagnosa";
		}
		$data=array(
	...
	{
		$global_set=array(
			'headline'=>'surat keterangan diagnosa',
			'url'=>$this->default_url,
		);
		$global=$this->global_set($global_set);

		//CEK SUBMIT DATA
		if($this->input->post('keterangandiagnosa_norm')){
			//PROSES SIMPAN
			$data=array(
				'keterangandiagnosa_nomor'=>$this->input->post('keterangandiagnosa_nomor'),
				'keterangandiagnosa_norm'=>$this->input->post('keterangandiagnosa_norm'),
				'keterangandiagnosa_nama'=>$this->input->post('keterangandiagnosa_nama'),
				'keterangandiagnosa_poli'=>$this->input->post('keterangandiagnosa_poli'),
				'keterangandiagnosa_tanggal'=>date('Y-m-d',strtotime($this->input->post('keterangandiagnosa_tanggal'))),
				'keterangandiagnosa_bulan'=>$this->duwi->bulannama(str_replace('0','',date('m',strtotime($this->input->post('keterangandiagnosa_tanggal'))))),
				'keterangandiagnosa_iduser'=>$this->session->userdata('user_id'),
			);
			########################################################
			// $file='berkas_file';
			// if($_FILES[$file]['name']){
	...
	public function tabel(){
		...
		// 	'tabel'=>'keterangandiagnosa a',
		// 	'join'=>[['tabel'=>'user b','ON'=>'b.user_id=a.keterangandiagnosa_iduser','jenis'=>'INNER']],
		// 	'order'=>array('kolom'=>'a.keterangandiagnosa_id','orderby'=>'DESC'),
		// );	
		$query='select a.*,b.user_nama from keterangandiagnosa a JOIN user b ON b.user_id=a.keterangandiagnosa_iduser ORDER BY LEFT(a.keterangandiagnosa_nomor,4) DESC';		
		$data=array(
			'global'=>$global,
	...
	public function add(){
		...
		$nomorurut=0;
		$global=$this->global_set($global_set);
		$db_nourut=$this->Crud->hardcode("SELECT LEFT(keterangandiagnosa_nomor,4) AS kode from keterangandiagnosa ORDER BY keterangandiagnosa_nomor DESC LIMIT 1")->row();
	...
	public function exportexcell(){
		...
		foreach($dt as $row) {
			...
			->setCellValue('B' . $kolom, $row->keterangandiagnosa_nomor)
			->setCellValue('C' . $kolom, $row->keterangandiagnosa_bulan)
			->setCellValue('D' . $kolom, $row->keterangandiagnosa_norm)
			->setCellValue('E' . $kolom, $row->keterangandiagnosa_nama)
			->setCellValue('F' . $kolom, $row->keterangandiagnosa_poli);
			$kolom++;
			$nomor++;
		}
		...
		if(isset($_FILES[$file]['name']) && in_array($_FILES[$file]['type'], $file_mimes)) {
			...
			for($i = 2;$i < count($sheetData);$i++)
			{
		    	...
		    	array_push($data, array(
					'keterangandiagnosa_nomor'=>$nomorsurat,
					'keterangandiagnosa_norm'=>$sheetData[$i]['4'],
					'keterangandiagnosa_nama'=>$sheetData[$i]['5'],
					'keterangandiagnosa_tanggal'=>date('Y-m-d'),
					'keterangandiagnosa_bulan'=>$sheetData[$i]['3'],
					'keterangandiagnosa_poli'=>$sheetData[$i]['6'],
					'keterangandiagnosa_iduser'=>$this->session->userdata('user_id'),
		    	));
		    }
			$query=array(
