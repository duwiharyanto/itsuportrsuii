...
class Suratvaksin extends CI_Controller {
// class Registrasi extends Core {
	...
	private $default_url="Nomorsurat/Suratvaksin/"; //Mendefinisikan url controller
	private $default_view="Nomorsurat/Suratvaksin/"; //Mendefinisiakn defaul view
	...
	private function global_set($data){
		...
		if(isset($data['overwriteview'])){
			...
			$overwriteview="views/Nomorsurat/Suratvaksin/index.php";
			$menu_submenu="surat_vaksin";
		}
		$data=array(
	...
	{
		$global_set=array(
			'headline'=>'surat keterangan vaksin',
			'url'=>$this->default_url,
		);
		$global=$this->global_set($global_set);

		//CEK SUBMIT DATA
		if($this->input->post('suratvaksin_norm')){
			//PROSES SIMPAN
			$data=array(
	...
	public function tabel(){
		...
		$query=array(
			...
			'tabel'=>'suratvaksin a',
			'join'=>[['tabel'=>'user b','ON'=>'b.user_id=a.suratvaksin_iduser','jenis'=>'INNER']],
			'order'=>array('kolom'=>'a.suratvaksin_id','orderby'=>'DESC'),
		);	
		$data=array(
			'global'=>$global,
	...
	public function add(){
		...
		$nomorurut=0;
		$global=$this->global_set($global_set);
		$db_nourut=$this->Crud->hardcode("SELECT LEFT(suratvaksin_nomor,4) AS kode from suratvaksin ORDER BY suratvaksin_nomor DESC LIMIT 1")->row();
	...
	public function exportexcell(){
		...
		foreach($dt as $row) {
			...
			->setCellValue('B' . $kolom, $row->suratvaksin_nomor)
