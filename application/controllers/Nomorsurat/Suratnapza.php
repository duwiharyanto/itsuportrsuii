...
class Suratnapza extends CI_Controller {
// class Registrasi extends Core {
	...
	//VARIABEL
	private $master_tabel="suratnapza"; //Mendefinisikan Nama Tabel
	private $id="suratnapza_id";	//Menedefinisaikan Nama Id Tabel
	private $kodeberkas="B.13";
	private $filename='Surat Keterangan Bebas Napza';
	private $namainstansi='RSUII';
	private $default_url="Nomorsurat/Suratnapza/"; //Mendefinisikan url controller
	private $default_view="Nomorsurat/Suratnapza/"; //Mendefinisiakn defaul view
	...
	private function global_set($data){
		...
		if(isset($data['overwriteview'])){
			...
			$overwriteview="views/Nomorsurat/Suratnapza/index.php";
	...
	{
		$global_set=array(
			'headline'=>'surat keterangan bebas napza',
			'url'=>$this->default_url,
		);
		$global=$this->global_set($global_set);

		//CEK SUBMIT DATA
		if($this->input->post('suratnapza_norm')){
			//PROSES SIMPAN
			$data=array(
				'suratnapza_nomor'=>$this->input->post('suratnapza_nomor'),
				'suratnapza_norm'=>$this->input->post('suratnapza_norm'),
				'suratnapza_nama'=>$this->input->post('suratnapza_nama'),
				'suratnapza_tanggal'=>date('Y-m-d',strtotime($this->input->post('suratnapza_tanggal'))),
				'suratnapza_bulan'=>$this->duwi->bulannama(str_replace('0','',date('m',strtotime($this->input->post('suratnapza_tanggal'))))),
				'suratnapza_iduser'=>$this->session->userdata('user_id'),
			);
			########################################################
			// $file='berkas_file';
			// if($_FILES[$file]['name']){
	...
	public function tabel(){
		...
		$query=array(
			...
			'tabel'=>'suratnapza a',
			'join'=>[['tabel'=>'user b','ON'=>'b.user_id=a.suratnapza_iduser','jenis'=>'INNER']],
			'order'=>array('kolom'=>'a.suratnapza_id','orderby'=>'DESC'),
		);	
		$data=array(
			'global'=>$global,
	...
	public function add(){
		...
		$nomorurut=0;
		$global=$this->global_set($global_set);
		$db_nourut=$this->Crud->hardcode("SELECT LEFT(suratnapza_nomor,4) AS kode from suratnapza ORDER BY suratnapza_nomor DESC LIMIT 1")->row();
	...
	public function exportexcell(){
		...
		foreach($dt as $row) {
			...
			->setCellValue('B' . $kolom, $row->suratnapza_nomor)
			->setCellValue('C' . $kolom, $row->suratnapza_bulan)
			->setCellValue('D' . $kolom, $row->suratnapza_norm)
			->setCellValue('E' . $kolom, $row->suratnapza_nama);
			$kolom++;
			$nomor++;
		}
		...
		if(isset($_FILES[$file]['name']) && in_array($_FILES[$file]['type'], $file_mimes)) {
			...
			for($i = 2;$i < count($sheetData);$i++)
			{
		    	...
		    	array_push($data, array(
					'suratnapza_nomor'=>$nomorsurat,
					'suratnapza_norm'=>$sheetData[$i]['4'],
					'suratnapza_nama'=>$sheetData[$i]['5'],
					'suratnapza_tanggal'=>date('Y-m-d'),
					'suratnapza_bulan'=>$sheetData[$i]['3'],
					'suratnapza_iduser'=>$this->session->userdata('user_id'),
		    	));
		    }
			$query=array(
