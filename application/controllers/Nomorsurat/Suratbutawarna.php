...
class Suratbutawarna extends CI_Controller {
// class Registrasi extends Core {
	...
	//VARIABEL
	private $master_tabel="suratbutawarna"; //Mendefinisikan Nama Tabel
	private $id="suratbutawarna_id";	//Menedefinisaikan Nama Id Tabel
	private $kodeberkas="B.1";
	private $filename='Surat Keterangan Bebas Buta warna';
	private $namainstansi='RSUII';
	private $default_url="Nomorsurat/Suratbutawarna/"; //Mendefinisikan url controller
	private $default_view="Nomorsurat/Suratbutawarna/"; //Mendefinisiakn defaul view
	...
	private function global_set($data){
		...
		if(isset($data['overwriteview'])){
			...
			$overwriteview="views/Nomorsurat/Suratbutawarna/index.php";
			$menu_submenu="surat_napza";
		}
		$data=array(
	...
	{
		$global_set=array(
			'headline'=>'surat bebas buta warna',
			'url'=>$this->default_url,
		);
		$global=$this->global_set($global_set);

		//CEK SUBMIT DATA
		if($this->input->post('suratbutawarna_norm')){
			//PROSES SIMPAN
			$data=array(
				'suratbutawarna_nomor'=>$this->input->post('suratbutawarna_nomor'),
				'suratbutawarna_norm'=>$this->input->post('suratbutawarna_norm'),
				'suratbutawarna_nama'=>$this->input->post('suratbutawarna_nama'),
				'suratbutawarna_tanggal'=>date('Y-m-d',strtotime($this->input->post('suratbutawarna_tanggal'))),
				'suratbutawarna_bulan'=>$this->duwi->bulannama(str_replace('0','',date('m',strtotime($this->input->post('suratbutawarna_tanggal'))))),
				'suratbutawarna_iduser'=>$this->session->userdata('user_id'),
			);
			########################################################
			// $file='berkas_file';
			// if($_FILES[$file]['name']){
	...
	public function tabel(){
		...
		$query=array(
			...
			'tabel'=>'suratbutawarna a',
			'join'=>[['tabel'=>'user b','ON'=>'b.user_id=a.suratbutawarna_iduser','jenis'=>'INNER']],
			'order'=>array('kolom'=>'a.suratbutawarna_id','orderby'=>'DESC'),
		);	
		$data=array(
			'global'=>$global,
	...
	public function add(){
		...
		$nomorurut=0;
		$global=$this->global_set($global_set);
		$db_nourut=$this->Crud->hardcode("SELECT LEFT(suratbutawarna_nomor,4) AS kode from suratbutawarna ORDER BY suratbutawarna_nomor DESC LIMIT 1")->row();
	...
	public function exportexcell(){
		...
		foreach($dt as $row) {
			...
			->setCellValue('B' . $kolom, $row->suratbutawarna_nomor)
			->setCellValue('C' . $kolom, $row->suratbutawarna_bulan)
			->setCellValue('D' . $kolom, $row->suratbutawarna_norm)
			->setCellValue('E' . $kolom, $row->suratbutawarna_nama);
			$kolom++;
			$nomor++;
		}
		...
		if(isset($_FILES[$file]['name']) && in_array($_FILES[$file]['type'], $file_mimes)) {
			...
			for($i = 2;$i < count($sheetData);$i++)
			{
		    	...
		    	array_push($data, array(
					'suratbutawarna_nomor'=>$nomorsurat,
					'suratbutawarna_norm'=>$sheetData[$i]['4'],
					'suratbutawarna_nama'=>$sheetData[$i]['5'],
					'suratbutawarna_tanggal'=>date('Y-m-d'),
					'suratbutawarna_bulan'=>$sheetData[$i]['3'],
					'suratbutawarna_iduser'=>$this->session->userdata('user_id'),
		    	));
		    }
			$query=array(
