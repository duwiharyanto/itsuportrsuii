...
class Core extends CI_Controller {
	...
	protected function listakses($levelakses){
		...
		$_aksesmenu=$this->Crud->read($menu)->result();	
	...
	protected function ceklogin(){
		...
		}elseif($this->session->userdata('user_level')!=1 && $this->session->userdata('user_login')==true){
			redirect(site_url($this->session->userdata('user_dashboard')));
		}
	}
	protected function nomorurut($param){
	...
	protected function bulanromawi($param){
		switch ($param) {
			case '1':
				$bulan='I';
				break;
	...
	protected function nomorsurat($param){
		//01.004/SMA-SM/V/2018
		//$nourut=$this->db->query("SELECT RIGHT(notest,4) AS kode from jual ORDER BY kode DESC LIMIT 1");
	...
	protected function gambarupload($path,$file){
		$config=array(
			'upload_path'=>$path,
	...
	protected function qrcode($param){
		$dir= "./barcode/";
	...
	protected function generateqrcode($param){
		include "./assets/phpqrcode/qrlib.php"; 
		$tempdir = $param['path'];
		#parameter inputan
		$isi_teks = $param['isi'];
